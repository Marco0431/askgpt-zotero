...
- 默认 **720 × 860**，比旧版更宽更高，空间几乎全给对话区；尺寸可在设置里改（也支持拖动标题栏移动，位置自动记忆）
- 顶部只有一行：模型徽章 + 两个状态徽章（`📄 整篇文献 12.3k 字 · PDF 全文`、`🎯 选中 41 字`）；读不到全文时第一个徽章变黄提示「仅发选中段落」
- 快捷提示 chips 单行横向滚动，顶部整体压缩到 ~59px，把高度让给对话
